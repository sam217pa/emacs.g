...
;;
;; dired-tar-dwim - If the file on the current line is a tar
...
;; - [2019-01-14 09:40] downloaded dired-tar from emacswiki.

;; TODO: support using compilation mode [2019-01-10 13:57]
...
(defvar dired-tar-should-gzip t
  "*t if dired-tar-dwim should gzip the files it creates, by default.
  ...
  (let ((buf (dired-tar-get-buffer)))
    (with-current-buffer buf
      ...
      (set (make-local-variable 'mode-line-process)
	       '(": %s"))
      (add-to-list 'compilation-finish-functions 'dired-tar-operation-done)
...
;;;###autoload
(defun dired-tar-dwim (prefix-arg)
...
(add-hook 'dired-mode-hook
	  (function
	   (lambda ()
	     (define-key dired-mode-map "T" 'dired-tar-dwim))))
